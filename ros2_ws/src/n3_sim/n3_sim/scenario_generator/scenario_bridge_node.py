...
class ScenarioBridgeNode(Node):
    ...
    async def _handle_ws_message(self, websocket, raw_msg: str) -> None:
        ...
        # wait_for_service and the call are blocking; run them in an executor so the asyncio loop (and WS connection) survives
    ...
    def _wait_for_future(self, future) -> ros.ScenarioCommand.Response | None:
        # Poll the future from an executor thread; the node already spins in main, so don't spin_until_future_complete again.
        import time
