...
namespace UnityEngine.Perception.GroundTruth
{
    class SimulationState
    {
        ...
        void ReportFramesToConsumer(bool flush = false, bool writeCapturesFromThisFrame = false)
        {
            ...
            // Local patch: write every frame that is ready, skipping (not blocking on)
            // any frame whose async annotations are still pending. Stock Perception
            // `break`s here to keep strict sequential order, but a single frame whose
            // GPU readback never completes (seen with the 3D bbox labeler on Metal)
            // then head-of-line-blocks every later frame and the whole next scenario.
            // SOLO frames are self-numbered, so out-of-order writes are fine; a stuck
            // frame stays pending and is flushed/logged at shutdown.
